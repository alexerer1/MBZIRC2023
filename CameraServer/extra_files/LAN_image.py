...
HOST = '167.0.0.1'  # The server's hostname or IP address
    ...
    while(1):
        ...
        if( x == '3'):
            ...
            while running:
                ...
                sockData = np.asarray(bytearray(nbytes), dtype="uint8")
                # sockData = np.fromstring(nbytes, np.uint8)
                
                print("Took ", (time.time()-start_t), " sec to get and image" )

                if result:
                    print(sockData.size)
                    cameraFeed = cv2.imdecode(sockData, 1)
